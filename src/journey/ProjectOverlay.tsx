...
export default function ProjectOverlay() {
  ...
  if (!p) return null
  return (
    <div className="fixed inset-0 z-30 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm" onClick={close}>
      <div
        role="dialog"
        aria-modal="true"
        aria-label={p.title}
      ...
      >
        <div className="flex items-start justify-between gap-4">
          ...
          <button ref={closeRef} onClick={close} aria-label="Close project details" className="rounded-full border border-white/15 px-3 py-1 text-sm hover:bg-white/10">✕</button>
        </div>
